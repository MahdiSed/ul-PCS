...
class UPCS():
    ...
    def Setup(self,F):
        ...
        for y in range(len(F)):
            ...
            for i in S[y]:
                ...
                W[y,i] = w_i
        Gamma1[1]=[[1,0,0],[0,-1,0],[0,1,0]]
        Gamma1[2]=[[1 if i==j else 0 for j in range(6)] for i in range(6)]; Gamma1[2][5][5]=-1
        Gamma1[3]=[[1,0],[0,-1]]
        Gamma2[1]=[[1,0,0],[0,-1,0],[0,1,0]]
        Gamma2[2]=[[1 if i==j else 0 for j in range(5)] for i in range(5)]; Gamma2[2][4][4]=-1
        msk={'sk_sigA':sk_sigA, 'sk_seqA':sk_seqA, 'A': A, 'W':W}
        mpk={'pp':pp, 'CRS1':CRS1, 'CRS2':CRS2, 'vk_sigA':vk_sigA, 'vk_seqA':vk_seqA, 'pp_com':pp_com, 'Gamma1':Gamma1, 'Gamma2':Gamma2}
        return (msk, mpk)
    ...
    def RandKey(self,mpk,sk):
        ...
        GS_comX[1], GS_comY[1] = NIZK.commit(self.NIZK,mpk['CRS1'],x,y,R_x,R_y,S_x,S_y)
        GS_proof[1] = NIZK.prove(self.NIZK,mpk['CRS1'],x,y,R_x,R_y,S_x,S_y,mpk['Gamma1'][1],GS_comX[1],GS_comY[1])
        ...
        (R_x,R_y,S_x,S_y)=NIZK.ParamGen(self.NIZK, x,y,c_a,c_b)
        GS_comX[2], GS_comY[2] = NIZK.commit(self.NIZK,mpk['CRS1'],x,y,R_x,R_y,S_x,S_y)
        GS_proof[2] = NIZK.prove(self.NIZK,mpk['CRS1'],x,y,R_x,R_y,S_x,S_y,mpk['Gamma1'][2],GS_comX[2],GS_comY[2])
        ...
        (R_x,R_y,S_x,S_y) = NIZK.ParamGen(self.NIZK, x,y,c_a,c_b)
        GS_comX[3], GS_comY[3] = NIZK.commit(self.NIZK,mpk['CRS1'],x,y,R_x,R_y,S_x,S_y)
        GS_proof[3] = NIZK.prove(self.NIZK,mpk['CRS1'],x,y,R_x,R_y,S_x,S_y,mpk['Gamma1'][3],GS_comX[3],GS_comY[3])
    ...
    def Sign(self,mpk,sk,pk_R,m,x):
        ...
        if SEQ.verify(self,mpk['pp'],mpk['vk_seqA'],pk_R['sigma_SEQ'],pk_R['M']) and \
            NIZK.verifyProof(self.NIZK,pp,mpk['CRS1'],pk_R['pi'],pk_R['comX'],pk_R['comY'],mpk['Gamma1']) and \
            ...
            while search==0:
                for key, value in sk[0]['W'].items():
                    if ACC.MemVrf(self,pp_p,pk_R['M'][1],x,value[0])==True:
                        ...
                        (R_x,R_y,S_x,S_y)=NIZK.ParamGen(self.NIZK, x,y,c_a,c_b)
                        GS_comX[1], GS_comY[1] = NIZK.commit(self.NIZK,mpk['CRS2'],x,y,R_x,R_y,S_x,S_y)
                        GS_proof[1] = NIZK.prove(self.NIZK,mpk['CRS2'],x,y,R_x,R_y,S_x,S_y,mpk['Gamma2'][1],GS_comX[1],GS_comY[1])
                        # SPS proof
                        ...
                        (R_x,R_y,S_x,S_y)=NIZK.ParamGen(self.NIZK, x,y,c_a,c_b)
                        GS_comX[2], GS_comY[2] = NIZK.commit(self.NIZK,mpk['CRS2'],x,y,R_x,R_y,S_x,S_y)
                        GS_proof[2] = NIZK.prove(self.NIZK,mpk['CRS2'],x,y,R_x,R_y,S_x,S_y,mpk['Gamma2'][2],GS_comX[2],GS_comY[2])
                        search=1
    ...
    def verify(self,mpk,pk_S,pk_R,m,sigma):
        ...
        if SEQ.verify(self,mpk['pp'],mpk['vk_seqA'],pk_S['sigma_SEQ'],pk_S['M']) and \
            SEQ.verify(self,mpk['pp'],mpk['vk_seqA'],pk_R['sigma_SEQ'],pk_R['M']) and \
                NIZK.verifyProof(self.NIZK,pp,mpk['CRS1'],pk_R['pi'],pk_R['comX'],pk_R['comY'],mpk['Gamma1']) and \
                NIZK.verifyProof(self.NIZK,pp,mpk['CRS1'],pk_S['pi'],pk_S['comX'],pk_S['comY'],mpk['Gamma1']) and \
            ...
            return DS.verify(self,mpk['pp'],pk_S['vk_sig'],sigma['sigma'],[m,pk_R['ID']]) and \
                 NIZK.verifyProof(self.NIZK,pp,mpk['CRS2'],pi_s['pi'],pi_s['comX'],pi_s['comY'],mpk['Gamma2'])
