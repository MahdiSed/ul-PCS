...
class UPCS():
    ...
    def Setup(self,N):
        ...
        CRS1 = NIZK.sampleParams(self.NIZK,pp)
        CRS2 = NIZK.sampleParams(self.NIZK,pp)
        (sk_sigA,vk_sigA) = SPS.keygen(self.SPS,pp,N+2)
        (sk_seq,vk_seq) = SEQ.keygen(pp,N+2) #For POK of FE ciphertext 
        param, gT = FE.G_IPE( pp,N) #OT12 pre setup
        ...
        mpk={'pp':pp, 'pp_com':pp_com, 'CRS1':CRS1, 'CRS2':CRS2, 'vk_sigA':vk_sigA,\
             'vk_seq':vk_seq, 'mpk_fe':mpk_fe, 'gT':gT, 'N':N, 'Gamma1':Gamma1,\
                'Gamma2':Gamma2, 'ck':ck, 'h':h}
        return (msk, mpk)
    ...
    def Sign(self,mpk,sk,pk_R,mes):
        ...
        result_fe=[1 for j in range(N) if Sigma.SingleGPC.Verify(self.Sigma,x_fe[j],pk_R['pi_fe'][j])==True]
        result_z=True
        ...
        NIZK.verifyProof(self.NIZK,pp,mpk['CRS1'],pk_R['pi'],pk_R['comX'],pk_R['comY'],mpk['Gamma1']) and \
            ...
            y.extend([sk[0]['sigma_sig2']['T'], pp['G2'], sk[0]['sigma_sig2']['T']])
    ...
    def verify(self,mpk,pk_S,pk_R,mes,sigma):
        ...
        if NIZK.verifyProof(self.NIZK,pp,mpk['CRS1'],pk_R['pi'],pk_R['comX'],pk_R['comY'],mpk['Gamma1']) and \
        ...
        return DS.verify(self.DS,pp,pk_S['vk_sig'],sigma['sigma'],[mes,pk_R['ID']]) and \
            NIZK.verifyProof(self.NIZK,pp,mpk['CRS2'],pi_s['pi'],pi_s['comX'],pi_s['comY'],mpk['Gamma2']) and \
